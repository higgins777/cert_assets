...
function apAnnualReportHideText() {
  ...
  if ($("#ap-initial-app-awaiting-pymnt").length > 0) {
...
function changeCeFormDates() {
  ...
  dateEndsCorrectly = reportingPeriod.text().includes("02/02/2026")
  ...
  if (isPending && isFiftyDue && dateEndsCorrectly && startDateCorrectly) {
      reportingPeriod.html("Reporting Period: 1/1/2024 - 02/02/2026");
  }
}
...
function changeNewSubmittalBtnLabel() {
  if ($("#aaFormSubmit_ATHLETIC_TRAINER").length == 0) return;
